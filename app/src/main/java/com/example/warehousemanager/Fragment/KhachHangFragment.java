package com.example.warehousemanager.Fragment;

import android.annotation.SuppressLint;
import android.app.Activity;
import android.app.AlertDialog;
import android.app.Dialog;
import android.os.Bundle;

import androidx.fragment.app.Fragment;
import androidx.recyclerview.widget.LinearLayoutManager;
import androidx.recyclerview.widget.RecyclerView;

import android.view.LayoutInflater;
import android.view.View;
import android.view.ViewGroup;
import android.view.animation.AnimationUtils;
import android.view.animation.LayoutAnimationController;
import android.widget.Button;
import android.widget.CheckBox;
import android.widget.EditText;
import android.widget.ImageView;
import android.widget.Toast;

import com.example.warehousemanager.Adapter.KhachHangAdapter;
import com.example.warehousemanager.Adapter.NhaCungCapAdapter;
import com.example.warehousemanager.Database.WaveHouseDB;
import com.example.warehousemanager.Model.KhachHang;
import com.example.warehousemanager.Model.NhaCungCap;
import com.example.warehousemanager.R;
import com.google.android.material.floatingactionbutton.FloatingActionButton;

import java.util.ArrayList;
import java.util.List;


public class KhachHangFragment extends Fragment {


    ImageView imgBack;
    EditText edTenKH , edSDT , edDiaChi ;
    Button btnSave ;
    RecyclerView rc_KH;
    FloatingActionButton fl_btnAdd;

    List<KhachHang> list = new ArrayList<>();
    KhachHangAdapter adapter;
    @Override
    public void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);

    }

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        // Inflate the layout for this fragment
        View view = inflater.inflate(R.layout.fragment_khach_hang, container, false);
        ((Activity)getContext()).setTitle("Khách Hàng");
        init(view);
        getData();
        openDialog();
        return view;
    }

    public void init(View view){
        rc_KH = view.findViewById(R.id.rc_khachHang);
        fl_btnAdd = view.findViewById(R.id.khachhang_btnAdd);
    }

    public void getData(){
        setLayoutAnimation(R.anim.layout_anim);
        list = WaveHouseDB.getInstance(getContext()).dao().getAllKhachHang();
        LinearLayoutManager manager = new LinearLayoutManager(getContext());
        rc_KH.setLayoutManager(manager);
        adapter = new KhachHangAdapter(getContext(), list);
        rc_KH.setAdapter(adapter);
    }
    private void setLayoutAnimation(int animResource){
        LayoutAnimationController layoutAnimationController = AnimationUtils.loadLayoutAnimation(getContext(),animResource);
        rc_KH.setLayoutAnimation(layoutAnimationController);
    }
    public void openDialog(){
        fl_btnAdd.setOnClickListener(new View.OnClickListener() {
            @SuppressLint("MissingInflatedId")
            @Override
            public void onClick(View v) {
                AlertDialog.Builder builder = new AlertDialog.Builder(getContext());
                View view = LayoutInflater.from(getContext()).inflate(R.layout.dialog_khach_hang, null);
                builder.setView(view);
                Dialog dialog = builder.create();
                dialog.show();
                KhachHang khachHang = new KhachHang();
                imgBack = view.findViewById(R.id.khachhang_imageBack);
                edTenKH = view.findViewById(R.id.khachhang_edTenKH);
                edSDT = view.findViewById(R.id.khachhang_edSoDT);
                edDiaChi = view.findViewById(R.id.khachhang_edDiaChi);
                btnSave = view.findViewById(R.id.khachhang_btnSave);
                btnSave.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {

                        if (edTenKH.getText().toString().isEmpty()) {
                            Toast.makeText(getContext(), "Tên không được để trống", Toast.LENGTH_SHORT).show();
                            return;
                        }
                        if (edSDT.getText().toString().isEmpty()) {
                            Toast.makeText(getContext(), "Số điện thoại không được để trống", Toast.LENGTH_SHORT).show();
                            return;
                        }
                        if (edDiaChi.getText().toString().isEmpty()) {
                            Toast.makeText(getContext(), "Địa chỉ không được để trống", Toast.LENGTH_SHORT).show();
                            return;
                        }

                        khachHang.setTenKhachHang(edTenKH.getText().toString().trim());
                        khachHang.setSoDT(edSDT.getText().toString().trim());
                        khachHang.setDiaChi(edDiaChi.getText().toString().trim());
                        if (WaveHouseDB.getInstance(getContext()).dao().addKhachHang(khachHang) > 0){
                            Toast.makeText(getContext(), "Thêm thành công", Toast.LENGTH_SHORT).show();
                            edTenKH.setText("");
                            edSDT.setText("");
                            edDiaChi.setText("");
                            getData();
                            dialog.dismiss();
                        }else{
                            Toast.makeText(getContext(), "Thêm không thành công", Toast.LENGTH_SHORT).show();
                            dialog.dismiss();
                        }
                    }
                });

                imgBack.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        edTenKH.setText("");
                        edSDT.setText("");
                        edDiaChi.setText("");
                        dialog.dismiss();
                    }
                });

            }
        });
    }
}